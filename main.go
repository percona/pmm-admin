...
func main() {
	...
	kingpin.Flag("listen-port", "Overwrite default listen port of pmm-agent").Default("7777").Uint32Var(&commands.GlobalFlags.ListenPort)
	...
	if command != commands.Status {
		commands.SetupClients(ctx, *serverURLF, commands.GlobalFlags.ListenPort)
	}
