...
ci-reviewdog:                   ## Runs reviewdog checks.
    ifneq ($(REVIEWDOG_GITHUB_API_TOKEN), )
		...
		echo "REVIEWDOG_GITHUB_API_TOKEN is not set, using golangci-lint without reviewdog"
		$(BIN_PATH)/golangci-lint run -c=.golangci.yml --out-format=line-number --new-from-rev=main
    endif
